...
import { useGetAllHospitalsQuery } from "@/app/store/slices/user.slice";
            ...
            <section className="messages-section my-5">
              <section className="message-area">
                ...
                <section className="all-messages my-5">
                  ...
                  ) : allHospitalsData?.data?.length === 0 ? (
                    <Text className="text-center text-sm text-gray-500 py-10">
                      No hospitals available to message
                    </Text>
                  ) : (
                    allHospitalsData?.data?.map((hospital: any) => {
                      return (
                        <Link
                          key={hospital._id}
                        ...
                        >
                          <section className="flex items-center gap-x-3 my-4 cursor-pointer hover:bg-blue-100 transition-colors duration-100 ease-in p-2 rounded-lg">
                            <div className={`avatar ${hospital?.online ? 'online' : 'offline'}`}>
                              <div className="w-14 rounded-full">
                                <img src={hospital?.profilePicture || '/medliink.png'} alt={hospital?.clinicName} />
                              </div>
                            </div>
                            <div className="message-content flex-1">
                              ...
                              <Text className="text-[12px] text-gray-600 truncate">
                                Start a conversation with {hospital?.clinicName}
                              </Text>
                            </div>
                          </section>
                        </Link>
